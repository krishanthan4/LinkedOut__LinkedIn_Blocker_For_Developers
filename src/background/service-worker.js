  ...
  if (request.action === 'ANALYZE_TEXT') {
    ...
    chrome.storage.sync.get(['grammarApiUrl', 'grammarApiKey'], async (data) => {
      ...
      try {
        ...
        // If apiKey/username provided for LT premium, append it. 
        ...
        // Parse LT response and construct full corrected text
        if (result.matches) {
           let correctedText = text;
           // Apply replacements in reverse order so offsets don't shift
           // sort matches by offset descending just in case
           ...
           // If no changes were made, or matches was empty
           if (correctedText === text) {
             sendResponse({ suggestions: ["Looks good! No errors found."] });
           } else {
             // Return the full corrected sentence(s)
             sendResponse({ suggestions: [correctedText] });
           }
        } else {
           // Maybe it is a different API type?
